...
<div class="container py-5">
    <div class="row mb-4">
        <div class="col-12">
            <h1 class="mb-3">Puestos de la Categoría: {{ $category }}</h1>
            
            @if($stands->count() > 0)
                <p class="text-muted">
                    Mostrando {{ $stands->firstItem() }}-{{ $stands->lastItem() }} de {{ $stands->total() }} puestos
                </p>
            @endif
        </div>
    </div>
    ...
    @if($stands->count() > 0)
        ...
        <div class="d-flex justify-content-center mt-4">
            {{ $stands->appends(['category' => $category])->links() }}
        </div>
    @else
        <div class="alert alert-info text-center">
            <p>No se encontraron puestos en la categoría "{{ $category }}".</p>
            <p>Prueba buscando en otras categorías o ajustando tus filtros.</p>
        </div>
    @endif
</div>
@endsection